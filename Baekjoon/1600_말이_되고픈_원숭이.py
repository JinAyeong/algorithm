# deque : 모든 가중치가 동일할때
# heap : 가중치가 다를때
# visited 배열 관리 방식: 좌표를 먼저 관리하고 말처럼 이동한 횟수는 나중에 처리하는 구조가 더 효율적입니다.
# -> [r][c][k+1] O , [k+1][r][c] X
# True/False 사용: Python의 부울형 처리가 더 빠르게 이루어질 수 있습니다.
# deque에 담는 값의 구조: 좌표 정보를 먼저 처리하는 것이 성능상 이점이 있습니다.
